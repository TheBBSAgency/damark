    ...
    <footer style={{ background: 'var(--steel-950)', color: 'var(--steel-300)' }}>
      <div style={{ maxWidth: 1240, margin: '0 auto', padding: '56px 32px 28px', display: 'grid', gridTemplateColumns: '1.4fr 1fr 1fr 1fr', gap: 32 }}>
        ...
        <div style={col}>
          ...
          <a style={link} href={window.href('service:cnc-precision-machining')}>CNC Precision Machining</a>
          <a style={link} href={window.href('service:fixturing-tooling')}>Fixturing &amp; Tooling</a>
          <a style={link} href={window.href('service:prototyping')}>Prototyping</a>
          <a style={link} href={window.href('service:reverse-engineering')}>Reverse Engineering</a>
          <a style={link} href={window.href('service:fabrication-welding')}>Fabrication &amp; Welding</a>
          <a style={link} href={window.href('service:shop-to-shop')}>Outsourced Machining for Shops</a>
          <a style={link} href={window.href('service:emergency-support')}>Emergency &amp; After-Hours</a>
        </div>
        <div style={col}>
          <div style={head}>Company</div>
          <a style={link} href={window.href('about')}>About</a>
          <a style={link} href={window.href('careers')}>Careers</a>
          <a style={link} href={window.href('contact')}>Contact</a>
        </div>
        <div style={col}>
